...
let connectionString = (config.NODE_ENV === 'production' || config.SIMULATE_PRODUCTION)
  ? config.DATABASE_URL
  : `postgresql://${config.PG_USER}:${config.PG_PASSWORD}@${config.PG_HOST}:${config.PG_PORT}/${config.PG_DATABASE}`

const pool = new Pool({
  connectionString,
  ... ((config.NODE_ENV === 'production' || config.SIMULATE_PRODUCTION) && { ssl: { rejectUnauthorized: false } }),
})
...
const addWindEntry = (data, request, response) => {
  pool.query(
    ...
    response.status(201).send(`Wind entry added for time: ${data.lastReceived}`)
  })
}

const getWindFromSource = () => {
  const windPromise = new Promise((resolve, reject) => {
    fetch("https://www.weatherlink.com/embeddablePage/getData/8ca3628b0cb24408bddddc7a166efd63", {
      "headers": {
        "accept": "*/*",
      ...
      "referrer": "https://www.weatherlink.com/embeddablePage/show/8ca3628b0cb24408bddddc7a166efd63/slim",
